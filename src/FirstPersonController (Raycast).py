...
class FirstPersonController(Entity):
    ...
    def update(self):
        ...
        self.distance = self.velocity.length() * time.dt

        # make sure we don't walk through walls
        for i in range(2):
            feet_ray = raycast(self.position+Vec3(0,0.5,0), self.velocity, ignore=(self,))
            head_ray = raycast(self.position+Vec3(0,self.height-.1,0), self.velocity, ignore=(self,))

            if feet_ray.hit:
                collision_normal = Vec3(feet_ray.world_normal.x, 0, feet_ray.world_normal.z).normalized()
                collision_distance = feet_ray.distance * collision_normal.dot(self.velocity.normalized())
                if -(self.distance+.5) < collision_distance < self.distance+.5:
                    self.velocity -= collision_normal * collision_normal.dot(self.velocity)
                    if i > 0: self.velocity = Vec3(0,0,0)

            elif head_ray.hit:
                collision_normal = Vec3(head_ray.world_normal.x, 0, head_ray.world_normal.z).normalized()
                collision_distance = head_ray.distance * collision_normal.dot(self.velocity.normalized())
                if -(self.distance+.5) < collision_distance < self.distance+.5:
                    self.velocity -= collision_normal * collision_normal.dot(self.velocity)
                    if i > 0: self.velocity = Vec3(0,0,0)
